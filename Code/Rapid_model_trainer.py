                    ...
                    for size in range (100, 201, 25):
                        for min_count in range(1, 10, 4):
                            for max_vocab in range(10000, 1000000, 1000):
                                ...
                                CBOW.get_model(language=language, hs=hs, negative=neg, cbow_mean=cbow_mean, iter=iter, size=size, min_count=min_count,
                                ...
                                Skip_gram.get_model(language=language, hs=hs, negative=neg, cbow_mean=cbow_mean, iter=iter, size=size,
                                           min_count=min_count,
                                           max_vocab_size=max_vocab, workers=3, articles_to_learn=1000000, randomTrain=True)
                                ...
                                Fast_Text.get_model(language=language, hs=hs, negative=neg, cbow_mean=cbow_mean, iter=iter, size=size, min_count=min_count,
                                            max_vocab_size=max_vocab, workers=3, articles_to_learn=1000000, randomTrain=True)
                                Fast_Text.save_model(model_name_generator('Fast_Text', [hs, neg, cbow_mean, iter, size, min_count], 1000000 ))

                                count +=1
                                print(count)
if __name__ == "__main__": train_models('English')
