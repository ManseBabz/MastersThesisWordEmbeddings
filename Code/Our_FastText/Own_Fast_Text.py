...
class FastText(BaseWordEmbeddingsModel):
    ...
    def __init__(self, sentences=None, sg=0, hs=0, size=100, alpha=0.025, window=5, min_count=5,
                 ...
                 bucket=2000000, trim_rule=None, batch_words=MAX_WORDS_IN_BATCH, callbacks=(), adaptive=1):
        ...
        adaptive : int 
            If 1 use adaptive version of ngrams.
        ...
        self.wv = FastTextKeyedVectors(size, min_n, max_n, adaptive)
    ...
    def build_ngrams(self, wv, update=False):
        if not update:
            ...
            for w, v in iteritems(wv.vocab):
                wv.ngrams_word[w] = _compute_ngrams(w, wv.min_n, wv.max_n, wv.adaptive)
        else:
            for w, v in iteritems(wv.vocab):
                wv.ngrams_word[w] = _compute_ngrams(w, wv.min_n, wv.max_n, wv.adaptive)
...
class FastTextTrainables(Word2VecTrainables):
    ...
    def init_ngrams_post_load(self, file_name, wv):
        ...
        for w, vocab in wv.vocab.items():
            all_ngrams += _compute_ngrams(w, wv.min_n, wv.max_n, wv.adaptive)
        ...
        for w, vocab in wv.vocab.items():
            word_ngrams = _compute_ngrams(w, wv.min_n, wv.max_n, wv.adaptive)
